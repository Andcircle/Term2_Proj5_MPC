...
int main() {
  ...
  MPC mpc;
  static double dt = 0.1; //control delay
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          int l=ptsx.size();
          Eigen::VectorXd map_x(l);
          Eigen::VectorXd map_y(l);
          ...
          for (int i=0;i<l;i++)
          {
        	  ...
        	  dx = ptsx[i] - px;
        	  dy = ptsy[i] - py;
        	  map_x[i] = dx * cospsi - dy * sinpsi;
        	  map_y[i] = dy * cospsi + dx * sinpsi;
          }

          Eigen::VectorXd coeffs = polyfit(map_x,map_y,1);
          ...
          double px_act = v * dt;
          double py_act = 0;
          double psi_act = - v * str * dt / Lf;
          double v_act = v + thr * dt;
...
//          double throttle_value=0;
          ...
          for(int i = 0; i<ptsx.size();i++){
			  next_x_vals.push_back(map_x[i]);
			  next_y_vals.push_back(map_y[i]);
          }
