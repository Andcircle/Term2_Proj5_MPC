...
const double w_cte = 10;
const double w_epsi = 0.5;
const double w_v = 0.5;
const double w_delta = 100000;
const double w_a = 15;
const double w_ddelta = 0.01;
const double W_da = 0.00001;

// Set lower and upper limits for variables.
const double max_str = 0.436332; // 25 deg in rad, used as delta bound
const double max_thr = 1.0;
const double no_limit = 1.0e3;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -no_limit;
    vars_upperbound[i] = no_limit;
  }
  // The upper and lower limits of delta are set to -25 and 25
  // degrees (values in radians).
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -max_str;
    vars_upperbound[i] = max_str;
  }
  // Acceleration/decceleration upper and lower limits
  for (int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -max_thr;
    vars_upperbound[i] = max_thr;
}
